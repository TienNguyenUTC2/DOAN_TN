    ...
    try {
      if (
        ...
        if (result?.length) {
          if (
            status === statuses.CANCEL.key &&
            (result[0].status === statuses.STARTED_USE.key ||
              result[0].status === statuses.ENDED_USE.key ||
              result[0].status === statuses.OTHER.key)
          ) {
            throw new APIError(
              403,
              "Bạn không được phép hủy khi đã nhận phòng"
            );
          }
          ...
          await this.handleUpdate({
            table: "rooms",
            data: {
              room_booking: room_booking < 0 ? 0 : room_booking,
              avaiable,
            },
            idField: "room_id",
            id: result[0].room_id,
